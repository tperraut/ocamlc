...
let rec resolve_expr (env: var_env) : Ast.expr -> Astv.expr = function
  ...
  (* DONE Cas à compléter : Eident, Eunop, Ebinop, Eif. *)
  | Eident i -> Astv.Evar (Env.find i env)
  | Eunop op, e-> Astv.Eunop (op, resolve_expr env e)
  | Ebinop op, e1, e2 -> Astv.Ebinop (op, resolve_expr env e)
  | Eif e1, e2, e3 -> Astv.Eif (resolve_expr env e1,
                                resolve_expr env e2,
                                resolve_expr env e3)
...
let rec resolve_instr (env: var_env) :
  ...
  = function
    ...
    (* TODO Cas à compléter : Idecl_var, Iassign, Iblock, Iwhile *)
    | Iassign (i, e)-> failwith "Not implemented"
    | Iblock b -> failwith "Not implemented"
    | Iwhile (e, b) -> failwith "Not implemented"
    | Idecl_var i ->
        let var = new_svar ... in
