...
let rec interpret_expr (env : venv) (fenv: fenv) : Ast.expr -> value
  ...
  | Eif (c, e1, e2) ->
    begin match interpret_expr env fenv c with
      ...
      | Bool _        -> interpret_expr env fenv e2
...
and interpret_instr ((env: venv), (fenv: fenv)) : instr -> venv * fenv
  ...
  | Iwhile (c, b)   ->
    while match interpret_expr env fenv c with
      | Bool b -> b
      | _      -> failwith "Expression mal formée"
    do ignore (interpret_block env fenv b) done; env, fenv
  | Iif (c, b1, b2) ->
    ignore (match interpret_expr env fenv c with
      | Bool b when b -> interpret_block env fenv b1
      | Bool _        -> interpret_block env fenv b2
      | _             -> failwith "Expression mal formée"
    );
    env, fenv
