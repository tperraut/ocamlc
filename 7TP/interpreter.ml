...
and interpret_instr ((env: venv), (fenv: fenv)) : instr -> venv * fenv
  = function
  | Idecl_var (id, _)    -> Env.add id (ref Undefined) env, fenv
  | Idecl_fun (f, ids, _, b) -> env, Env.add f (ids, b) fenv
  ...
  | Iif (c, b1, b2) ->
    begin match interpret_expr env fenv c with
      | Bool b when b -> ignore (interpret_block env fenv b1); env, fenv
      | Bool b        -> ignore (interpret_block env fenv b2); env, fenv
      | _             -> failwith "Expression mal formée"
    end
